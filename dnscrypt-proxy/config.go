...
// DNSSECValidationConfig controls whether answers are checked against the
// signatures the zone published.
type DNSSECValidationConfig struct {
	// Mode is "off", "log" or "enforce". Enforcing a validator that has never
	// run is how a resolver goes dark, so the useful order is log first, read
	// what it says about the traffic that is actually seen, then enforce.
	Mode string `toml:"mode"`
	// InsecureZones are treated as unsigned however the chain reads them, for
	// a zone whose signing is known to be broken. Each entry disables the
	// protection for everything at or below it.
	InsecureZones []string `toml:"insecure_zones"`
}
...
type Config struct {
	...
	DNSSECValidation         DNSSECValidationConfig `toml:"dnssec_validation"`
	Cache                    bool
...
func newConfig() Config {
	return Config{
		DNSSECValidation: DNSSECValidationConfig{Mode: "off"},
...
func ConfigLoad(proxy *Proxy, flags *ConfigFlags) error {
	...
	proxy.dnssecValidationMode = config.DNSSECValidation.Mode
	proxy.dnssecInsecureZones = config.DNSSECValidation.InsecureZones
